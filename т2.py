...
class ActionManager:
    def __init__(self):
        ...
        self.screen_width = self.current_resolution.width
        self.screen_height = self.current_resolution.height
        self.dirty = False
        self.load_actions()
...
class BotRunner:
    def __init__(self, main_window):
        ...
        self.profile_check_buttons = main_window.profile_check_buttons
        self.action_manager = main_window.action_manager
        self.root = main_window.root
        self.progress_label = main_window.progress_label
        self.progress_bar = main_window.progress_bar
        self.status_label = main_window.status_label
        self.stop_button = main_window.stop_button

    def get_action_delay(self, action):
        if action['type'] == "натискання_клавіші" and action.get("key") == "enter":
            return action.get("delay", DEFAULT_CONFIG["action_delay"]["enter_key"])
        return action.get("delay", DEFAULT_CONFIG["action_delay"].get(action['type'], 2.0))

    def handle_text_input(self, action, x, y):
        pyautogui.click(x, y)
        time.sleep(0.5)
    ...
    def check_resolution(self, template_name):
        current_resolution = self.action_manager.current_resolution
        if current_resolution.width != self.action_manager.screen_width or current_resolution.height != self.action_manager.screen_height:
            logger.warning(f"Роздільна здатність змінилась: збережено {self.action_manager.screen_width}x{self.action_manager.screen_height}, поточна {current_resolution.width}x{current_resolution.height}")
            self.status_label.config(text="Роздільна здатність екрана змінилась")

    def start(self, selected_templates, selected_profiles):
        self.running = True
        ...
        for template_name in selected_templates:
            ...
            logger.debug(f"Кількість дій у шаблоні {template_name}: {len(template['actions'])}")
            if not template["profiles"]:
                ...
                return
            total_actions += len(template["actions"]) * len([p for p in selected_profiles if p in template["profiles"]])
        current_action = [0]
        ...
        def run_profile(profile_name, template_name, callback):
            ...
            if not shortcut_path or not os.path.exists(shortcut_path):
                ...
                self.action_manager.dirty = True
                self.action_manager.save_actions()
                self.main_window.update_profiles_list()
            ...
            def execute_action(action, action_callback, retries=2, index=0):
                if not self.running or profile_name not in action.get("profiles", []):
                    logger.info(f"Дія пропущена: {action['action']}, профіль {profile_name} не в {action.get('profiles', [])}")
                    action_callback()
                    return
                ...
                try:
                    ...
                    action_handlers = {
                        "подвійний_клік": pyautogui.doubleClick,
                        "клік": pyautogui.click,
                        "введення_тексту": lambda: self.handle_text_input(action, x, y),
        ...
        def run_template(template_index=0):
            ...
            template_profiles = [profile for profile, var in self.main_window.profile_check_vars.items()
                                if var.get() and profile in self.action_manager.data["templates"][template_name]["profiles"]]
            ...
            def run_all_profiles(profile_index=0):
                if not self.running or profile_index >= len(template_profiles):
                    logger.info(f"Завершено шаблон: {template_name}")
                    run_template(template_index + 1)
                    return
                run_profile(template_profiles[profile_index], template_name,
...
class MainWindow:
    ...
    def update_mouse_coords(self):
        if self.mouse_tracking and not self.recording:
            ...
            self.root.after(200, self.update_mouse_coords)
    ...
    def setup_record_tab(self):
        ...
        self.move_action_button = tk.Button(button_frame, text="Вгору", command=lambda: self.move_action("up"),
                                        font=DEFAULT_CONFIG["default_font"], width=7)
        self.move_action_button.pack(side=tk.LEFT, padx=1)
        Tooltip(self.move_action_button, "Перемістити дію вгору у списку")
        self.move_down_button = tk.Button(button_frame, text="Вниз", command=lambda: self.move_action("down"),
                                          font=DEFAULT_CONFIG["default_font"], width=7)
        ...
        def save_binding():
            ...
            if selected_profiles:
                for template_name in template_names:
                    ...
                    for action in self.action_manager.data["templates"][template_name]["actions"]:
                        action["profiles"] = selected_profiles
            self.action_manager.dirty = True
            self.action_manager.save_actions()
            self.update_profiles_list()
            self.status_label.config(text=f"Профілі прив’язані до {', '.join(template_names)}")
            logger.info(f"Прив’язані профілі до {selected_profiles}: {template_names}")
            bind_window.destroy()
    ...
    def start_recording(self):
        ...
        self.move_action_button.config(state=tk.DISABLED)
        ...
        def monitor_recording():
            if not self.recording:
                ...
                self.move_action_button.config(state=tk.NORMAL)
    ...
    def fix_action(self):
        ...
        current_resolution = {"width": self.action_manager.current_resolution.width, "height": self.action_manager.current_resolution.height}
        x = min(max(0, x), current_resolution["width"] - 1)
        y = min(max(0, y), current_resolution["height"] - 1)
        show_checkmark(x, y)
    ...
    def move_action(self, direction="up"):
        ...
        if (direction == "up" and action_index == 0) or (direction == "down" and action_index >= max_index):
            messagebox.showinfo("Інформація", f"Дія вже {'на початку' if direction == 'up' else 'в кінці'} списку!")
            logger.info(f"Спроба перемістити дію {direction} на межі списку")
            return
        new_index = action_index - 1 if direction == "up" else action_index + 1
        try:
            for template_name in selected_templates:
                actions = self.action_manager.data["templates"][template_name]["actions"]
                actions[action_index], actions[new_index] = actions[new_index], actions[action_index]
                logger.info(f"Переміщено дію {actions[new_index]['action']} з {action_index + 1} на {new_index + 1} у {template_name}")
            self.action_manager.dirty = True
            self.action_manager.save_actions()
            self.update_actions_list()
            self.actions_list.selection_clear(0, tk.END)
            self.actions_list.selection_set(new_index)
            self.status_label.config(text=f"Дію переміщено {'вгору' if direction == 'up' else 'вниз'}")
        except Exception as e:
            log_error(f"Помилка переміщення дії {direction}", e)
    ...
    def start_bot(self):
        selected_templates = [name for name, var in self.run_template_vars.items() if var.get()]
        selected_profiles = [name for name, var in self.profile_check_vars.items() if var.get()]
        self.bot_runner.start(selected_templates, selected_profiles)
